...
/**
 * Process the HSET command (see https://redis.io/commands/hset).
 * @param hash The hash.
 * @param key The key.
 * @param value The assigned value.
 * @return If it's okay.
 */
bool Redis::hset(const char *hash, const char *key, const char *value)
{
    this->conn.println("*4");
    this->conn.println("$4");
    this->conn.println("HSET");
    this->conn.print("$");
    this->conn.println(strlen(hash));
    this->conn.println(hash);
    this->conn.print("$");
    this->conn.println(strlen(key));
    this->conn.println(key);
    this->conn.print("$");
